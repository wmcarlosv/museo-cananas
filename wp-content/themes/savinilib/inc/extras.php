 ...
 * @return array
 */
function savinilib_page_menu_args( $args ) {
	$args['show_home'] = true;
	return $args;
}
add_filter( 'wp_page_menu_args', 'savinilib_page_menu_args' );
 ...
 * @return array
 */
function savinilib_body_classes( $classes ) {
	// Adds a class of group-blog to blogs with more than 1 published author.
	if ( is_multi_author() ) {
		$classes[] = 'group-blog';
	}

	if ( get_theme_mod( 'savinilib_sidebar_position' ) == 'pull-right' ) {
		$classes[] = 'has-sidebar-left';
	} elseif ( get_theme_mod( 'savinilib_sidebar_position' ) == 'no-sidebar' ) {
		$classes[] = 'has-no-sidebar';
	} elseif ( get_theme_mod( 'savinilib_sidebar_position' ) == 'full-width' ) {
...
add_filter( 'body_class', 'savinilib_body_classes' );


// Mark Posts/Pages as Untiled when no title is used
add_filter( 'the_title', 'savinilib_title' );

function savinilib_title( $title ) {
	if ( '' == $title ) {
		return __( 'Untitled', 'savinilib' );
	} else {
		return $title;
	}
}
 ...
 */
add_filter( 'the_password_form', 'savinilib_custom_password_form' );

function savinilib_custom_password_form() {
	global $post;
	$label = 'pwbox-' . ( empty( $post->ID ) ? rand() : $post->ID );
	$o = '<form class="protected-post-form" action="' . get_option( 'siteurl' ) . '/wp-login.php?action=postpass" method="post">
			<div class="row">
				<div class="col-lg-10">
					<p>' . esc_html__( 'This post is password protected. To view it please enter your password below:' ,'savinilib' ) . '</p>
					<label for="' . $label . '">' . esc_html__( 'Password:' ,'savinilib' ) . ' </label>
					<div class="input-group">
						<input class="form-control" value="' . get_search_query() . '" name="post_password" id="' . $label . '" type="password">
						<span class="input-group-btn"><button type="submit" class="btn btn-default" name="submit" id="searchsubmit" value="' . esc_attr__( 'Submit','savinilib' ) . '">' . esc_html__( 'Submit' ,'savinilib' ) . '</button></span>
					</div>
				</div>
			</div>
		</form>';
...
add_filter( 'the_content', 'savinilib_add_custom_table_class' );
function savinilib_add_custom_table_class( $content ) {
	return str_replace( '<table>', '<table class="table table-hover">', $content );
}

if ( ! function_exists( 'savinilib_header_menu' ) ) :
	/**
 * Header menu (should you choose to use one)
 */
	function savinilib_header_menu() {
		...
		wp_nav_menu( array(
			...
			'fallback_cb'       => 'savinilib_Wp_Bootstrap_Navwalker::fallback',
			'walker'            => new savinilib_Wp_Bootstrap_Navwalker(),
		));
	}
endif;

if ( ! function_exists( 'savinilib_featured_slider' ) ) :
	/**
 * Featured image slider, displayed on front page for static page and blog
 */
	function savinilib_featured_slider() {
		if ( ( is_home() || is_front_page() ) && get_theme_mod( 'savinilib_featured_hide' ) == 1 ) {
			...
			$slidecat = get_theme_mod( 'savinilib_featured_cat' );
			$slidelimit = get_theme_mod( 'savinilib_featured_limit', -1 );
			$slider_args = array(
				'cat' => $slidecat,
					...
					if ( ( function_exists( 'has_post_thumbnail' ) ) && ( has_post_thumbnail() ) ) :
						...
						if ( class_exists( 'Jetpack' ) && Jetpack::is_module_active( 'photon' ) ) {
							...
							echo '<img src="' . $photon_url . '">';
							  ...
							  echo get_the_post_thumbnail( get_the_ID(), 'savinilib-slider' );
						}
								echo '<div class="flex-caption">';
							  echo get_the_category_list();
						if ( get_the_title() != '' ) { echo '<a href="' . get_permalink() . '"><h2 class="entry-title">' . get_the_title() . '</h2></a>';
						}
								echo '<div class="read-more"><a href="' . get_permalink() . '">' . __( 'Read More', 'savinilib' ) . '</a></div>';
								echo '</div>';
								echo '</li>';
						endif;
					endwhile;
 ...
 */
function savinilib_footer_info() {
	global $savinilib_footer_info;
	printf( esc_html__( 'Theme by %1$s Powered by %2$s', 'savinilib' ) , '<a href="http://colorlib.com/" target="_blank">Colorlib</a>', '<a href="http://wordpress.org/" target="_blank">WordPress</a>' );
}
 ...
 */
function savinilib_caption( $output, $attr, $content ) {
	if ( is_feed() ) {
		return $output;
	}
...
add_filter( 'img_caption_shortcode', 'savinilib_caption', 10, 3 );

/**
 * Skype URI support for social media icons
 */
function savinilib_allow_skype_protocol( $protocols ) {
	$protocols[] = 'skype';
	return $protocols;
}
add_filter( 'kses_allowed_protocols' , 'savinilib_allow_skype_protocol' );

/*
 * This display blog description from wp customizer setting.
 */
function savinilib_cats() {
	$cats = array();
	$cats[0] = 'All';
 ...
 */
function savinilib_cb_comment( $comment, $args, $depth ) {
...
?>
	...
	<div class="comment-author vcard asdasd">
		...
		<?php printf( __( '<cite class="fn">%s</cite> <span class="says">says:</span>', 'savinilib' ), get_comment_author_link() ); ?>
		<?php
		...
		<div class="comment-meta commentmetadata"><a href="<?php echo htmlspecialchars( get_comment_link( $comment->comment_ID ) ); ?>">
			...
			printf( __( '%1$s at %2$s', 'savinilib' ), get_comment_date(), get_comment_time() ); ?></a><?php edit_comment_link( __( 'Edit', 'savinilib' ), '  ', '' );
			?>
		</div>
	</div>

	<?php if ( '0' == $comment->comment_approved ) : ?>
		<em class="comment-awaiting-moderation"><?php _e( 'Your comment is awaiting moderation.', 'savinilib' ); ?></em>
		<br />
	<?php endif; ?>
 ...
 */
if ( ! function_exists( 'get_savinilib_theme_setting' ) ) {
	function get_savinilib_theme_setting() {
...
add_action( 'wp_head', 'get_savinilib_theme_setting', 10 );

/**
 * Adds the URL to the top level navigation menu item
 */
function savinilib_add_top_level_menu_url( $atts, $item, $args ) {
	if ( ! wp_is_mobile() && isset( $args->has_children ) && $args->has_children ) {
		$atts['href'] = ! empty( $item->url ) ? $item->url : '';
	}
	return $atts;
}
add_filter( 'nav_menu_link_attributes', 'savinilib_add_top_level_menu_url', 99, 3 );

/**
 * Makes the top level navigation menu item clickable
 */
function savinilib_make_top_level_menu_clickable() {
	if ( ! wp_is_mobile() ) { ?>
...
add_action( 'wp_footer', 'savinilib_make_top_level_menu_clickable', 1 );
